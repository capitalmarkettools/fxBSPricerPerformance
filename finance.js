...
Meteor.methods({
    BlackScholes(PutCallFlag, S, X, T, r, v) {
        ...
        if (PutCallFlag === "call") {
            return ( S * Meteor.call('CND', d1) - X * Math.exp(-r * T) * Meteor.call('CND',d2) );
        } else {
            return ( X * Math.exp(-r * T) * Meteor.call('CND', -d2) - S * Meteor.call('CND',-d1) );
        }
    },
